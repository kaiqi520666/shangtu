    ...
    <div class="flex w-full flex-col items-center gap-3">
      <div v-for="nav in mainNavs" :key="nav.to" class="group relative flex w-full flex-col items-center">
        <RouterLink
          ...
          :aria-label="nav.name"
          :class="$route.path === nav.to ? 'border border-primary/20 bg-primary/10 text-primary shadow-sm' : 'text-slate-500 hover:bg-white hover:text-slate-800 hover:shadow-sm'"
        >
          <span v-if="$route.path === nav.to" class="absolute -left-[15px] top-1/2 h-7 w-1 -translate-y-1/2 rounded-r-full bg-primary"></span>
          <component :is="nav.icon" class="h-5 w-5 stroke-[2.2]" />
        </RouterLink>
        <span class="mt-1 max-w-[64px] truncate text-center text-xs font-medium leading-none" :class="$route.path === nav.to ? 'text-primary' : 'text-slate-500'" :title="nav.name">
          {{ nav.name }}
        </span>
