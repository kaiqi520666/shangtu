...
const mainNavs = [
  ...
  { name: '自由生图', desc: '输入提示词，自由生成或参考图片改图', icon: Sparkles, to: '/generator/free-image', dividerAfter: true },
  ...
  { name: 'AI配音', desc: '预置音色生成 MP3 配音', icon: AudioLines, to: '/generator/voiceover', dividerAfter: true },
...
<template>
  <aside class="flex w-16 flex-col items-center border-r border-slate-200 bg-slate-50 py-3">
    <div class="flex min-h-0 w-full flex-1 flex-col items-center gap-1 overflow-y-auto">
      <template v-for="nav in mainNavs" :key="nav.to">
        ...
        <div v-if="nav.dividerAfter" class="my-2 h-px w-7 shrink-0 bg-slate-200"></div>
      </template>
      <template v-if="authStore.isSuperAdmin">
        <div class="my-2 h-px w-7 shrink-0 bg-slate-200"></div>
        <div class="flex w-full justify-center">
          <RouterLink
            :to="adminNav.to"
            class="flex h-12 w-12 items-center justify-center rounded-xl transition-colors duration-200"
