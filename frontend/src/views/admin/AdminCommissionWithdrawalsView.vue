...
<template>
  <AdminCommissionWithdrawalsPanel :state="distribution.state" @update-keyword="distribution.state.keyword = $event" @update-status="distribution.state.status = $event" @filter="distribution.applyFilter" @page="distribution.changePage(distribution.state, distribution.load, $event)" @approve="distribution.approve" @reject="distribution.openAction('reject', $event)" @pay="distribution.openAction('pay', $event)" />
